<?php
	...
	if ($db->num_rows($r) == 0)
		admin_load("Không tồn tại Mục này.","?act=evt_list&id=".$id);
	...
	if ($func == "del")
	{
		...
		admin_load("Đã xóa các Bài viết đã chọn.","?act=evt_list&id=".$id);
		die();
	}
...
<div class="page-title">
  ...
  <div>
    <ul class="breadcrumb">
      ...
      <li><a href="?act=evt_manager">Quản lý nội dung</a></li>
    </ul>
  </div>
</div>
<div class="row">
	<div class="col-md-12">
		<div class="card">
			...
			<div class="table-responsive">
				<form action="?act=evt_list&id=<?=$_GET['id']?>" id="frm_sanpham" enctype="multipart/form-data" method="post">


					<a href="?act=evt_new&txt_cat=<?=$id?>" class="btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</a>
						...
						<tbody>
							...
							<tr class="tb_content">
							    ...
							    <td align="center"><?if($row['hinh']=="no"){
							        ...
							        <?}else{?>
							        <img src="../uploads/evt/news_<?=$row['hinh']?>" width="40px" class="img-responsive"/>    
							        <?}
							        ?>
							    </td>
								<td style="text-align:left !important;"><a href="?act=evt_edit&id=<?=$row["id"]?>" style="text-decoration:none;"><?=$row["ten"]?></a></td>
								...
								<td align="center"><a href="?act=evt_edit&id=<?=$row["id"]?>" class="icon-form"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td>
							...
							<?php
					    		for($i=1;$i<=$pages;$i++) {
					    			if (($_GET['page'] == $i) ||(!isset($_GET['page']) && $i == 1))
					    				echo "<li class=\"active\"><a href='?act=evt_list&id=".$id."&page=$i'>".$i."</a></li>";
					    			else
					    				echo "<li><a href='?act=evt_list&id=".$id."&page=$i'>".$i."</a></li>";
								}
					    	?>
	                  </ul>
					</div>
