<?php
	...
	if ($func == "del")
	{
		...
		admin_load("Đã xóa các Bài viết đã chọn.","?act=sanpham_list&id=".$_POST["id"]);
		die();
	}
...
<div class="page-title">
  <div>
    <h1><i class="fa fa-dashboard"></i> Quản lý phòng</h1>
    <p></p>
  </div>
  <div>
    <ul class="breadcrumb">
					...
					<table class="table table-striped table-bordered">
						...
						<tbody>
							<?php 
								...
								$perpage	=	20;
								if ($id!=''&&$id!='0') {
									$r_all		=	$db->select("sanpham","cat = '".$id."'");
								} else {
									$r_all		=	$db->select("sanpham","");
								}
								...
								$count	=	$min;
								if ($id!=''&&$id!='0') {
									$r = $db->select("sanpham","cat = '".$id."'","order by time desc, id desc limit $min, $max");
								} else {
									$r = $db->select("sanpham","","order by time desc, id desc limit $min, $max");
								}
								...
								<tr class="tb_content">
								    ...
								    <td style="font-weight: bold; text-align: left;"><a style="text-decoration:none" href="?act=sanpham_edit&id=<?=$row["id"]?>"><?=$row["ten"]?></a>
								    <!-- <small>
