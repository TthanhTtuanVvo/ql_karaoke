...
<?php
	...
	if ($func == "new")
	{
		...
		else
		{
			...
			if ($OK)
			{
				...
				$id = $db->insert("tgp_cms","id,cat,ten,time_event,chu_thich,hinh_note,noi_dung,hien_thi,noi_bat,time,user, seo_title, seo_keyword, seo_description","0,'".($txt_cat+0)."','".$db->escape($txt_ten)."','".strtotime($txt_time)."','".$txt_chu_thich."','".$db->escape($txt_hinh_note)."','".$txt_noi_dung."','".($txt_hien_thi+0)."','".($txt_noi_bat+0)."','".$time."','".$thanh_vien["id"]."','".($txt_title)."','".($txt_keyword)."','".($txt_description)."'");
	...
	if (!$OK)
		template_edit("?act=evt_new", "new", 0 ,$txt_cat,$txt_ten,$txt_time,$txt_chu_thich,$txt_hinh,$txt_hinh_note,$txt_noi_dung,$txt_hien_thi,$txt_noi_bat,$txt_date,$error,$txt_title,$txt_keyword,$txt_description)
?>
</center>