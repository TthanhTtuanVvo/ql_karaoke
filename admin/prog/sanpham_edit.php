...
<?php
	...
	if ($func == "update")
	{
		...
		else
		{
			...
			if ($OK)
			{
				$db->query("update sanpham set cat = '".$txt_cat."',ten = '".$db->escape($txt_ten)."',chu_thich = '".$db->escape($txt_chu_thich)."', noi_dung = '".$txt_noi_dung."', hien_thi = '".($txt_hien_thi+0)."' , noi_bat = '".($txt_noi_bat+0)."' ,time = '".time()."', seo_title = '".($txt_title)."', seo_keyword = '".($txt_keyword)."', seo_description = '".($txt_description)."' where id = '".$id."'");
				if ($hinh)
				{
				...
				admin_load("Đã cập nhật thành công.","?act=sanpham_list&id=".$txt_cat);
			}			
		}
	}
	else
	...
	if (!$OK)
		template_edit("?act=sanpham_edit","update",$id,$txt_cat,$txt_ten,$txt_chu_thich,$txt_noi_dung,$txt_hien_thi,$txt_noi_bat, $txt_title, $txt_keyword, $txt_description,$error)
?>
</center>