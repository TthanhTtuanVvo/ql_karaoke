<?php
    ...
    include "templates/evt_menu.php";
	if (empty($func)) $func = $_POST['func'];
	$id = $id+0;
	//	Kiểm tra sự tồn tại của ID
	$r	= $db->select("tgp_cms_menu","id = '".$id."'");
	if ($db->num_rows($r) == 0)
		admin_load("Không tồn tại mục này.","?act=evt_manager");
?>
<center>
<?php
	$OK = false;
	...
	if ($func == "update")
	{
		...
		else
		{
			...
			if ($OK)
			{
				...
				admin_load("Cập nhật thành công.","?act=evt_manager");
				$OK = true;
			}
		}
	}
	...
	if (!$OK)
		template_edit("?act=evt_menu_edit&id=".$id,"update",$id,$txt_cat,$txt_ten,$txt_hien_thi,$txt_type,$txt_noi_bat,$error,$txt_title,$txt_keyword,$txt_description);
?>
</center>