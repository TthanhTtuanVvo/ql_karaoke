...
<?php
	...
	if ($func == "new")
	{
		...
		else
		{
			...
			if ($OK)
			{
				$id = $db->insert("vatlieu","ten,chu_thich,noi_dung,hien_thi,noi_bat,time,user,seo_title, seo_keyword, seo_description","'".$db->escape($txt_ten)."','".$txt_chu_thich."','".$txt_noi_dung."','".($txt_hien_thi+0)."','".($txt_noi_bat+0)."','".time()."','".$thanh_vien["id"]."','".$txt_title."','".$txt_keyword."','".$txt_description."'");
				...
				admin_load("Đã thêm mới vào CSDL","?act=phong_list&id=".$txt_cat);
			}
		}
	}
	else
	...
	if (!$OK)
		...
		template_edit("?act=phong_new", "new", 0 ,$txt_ten,$txt_chu_thich,$txt_noi_dung,$txt_hien_thi,$txt_noi_bat, $txt_title, $txt_keyword, $txt_description,$error)
